#!/usr/bin/env python3
"""Run Moshi v3 final stages only after an eligible selection is committed."""
    ...
    return [
        {
            ...
            "test_access": False,
            "output": ROOT / "results/moshi_v3_adapter_validation.json",
            "command": [
                moshi_python,
                ...
                "--out",
                "results/moshi_v3_adapter_validation.json",
                "--runtime-device",
                "cuda",
            ],
        },
        {
            ...
            "test_access": True,
            "output": ROOT / "results/moshi_v3_final_test.json",
            "command": [
                moshi_python,
                ...
                "--out",
                "results/moshi_v3_final_test.json",
                "--experiment-label",
                "v3",
            ],
        },
        {
            ...
            "test_access": True,
            "output": ROOT / "results/moshi_v3_final_runtime.json",
            "command": [
                moshi_python,
                ...
                "results/moshi_v2_split.json",
                "--out",
                "results/moshi_v3_final_runtime.json",
                "--experiment-label",
                "v3",
            ],
        },
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("results/hardware/moshi_v3_final_pipeline.json"),
    )
    args = parser.parse_args()
    out_path = (ROOT / args.out).resolve()
    if out_path.exists():
        raise FileExistsError(f"refusing to overwrite v3 final receipt: {out_path}")

    selection_path = ROOT / "results/moshi_v3_checkpoint_selection.json"
    validation_pipeline_path = ROOT / "results/hardware/moshi_v3_validation_pipeline.json"
    training_certificate_path = ROOT / "results/hardware/moshi_h100_v3_training.json"
    ...
    if not all(preconditions.values()):
        raise RuntimeError(f"v3 final-pipeline preconditions failed: {preconditions}")
    ...
    for stage in final_commands():
        ...
        if output.exists():
            raise FileExistsError(f"refusing to overwrite v3 final output: {output}")
