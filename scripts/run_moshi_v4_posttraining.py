#!/usr/bin/env python3
"""Run the Moshi v4 validation-only chain and stop before final-test access."""
    ...
    return [
        {
            ...
            "output": ROOT / "results/moshi_v4_validation_reevaluation.json",
            "command": [
                moshi_python,
                ROOT / "scripts/reevaluate_moshi_checkpoints.py",
                "--training-config",
                "configs/moshi_h100_v4.yaml",
                "--run-dir",
                "checkpoints/moshi_fa_100s_v4_text_embed",
                "--metrics-out",
                "checkpoints/moshi_fa_100s_v4_text_embed/metrics.reeval.jsonl",
                "--report-out",
        ...
        },
        {
            ...
            "output": ROOT / "results/moshi_v4_runtime_candidates.json",
            "command": [
                moshi_python,
                ROOT / "scripts/evaluate_moshi_v4_runtime_candidates.py",
            ],
        },
        {
            "name": "eligible_only_selection",
            "output": ROOT / "results/moshi_v4_checkpoint_selection.json",
            "command": [
                project_python,
...
def main() -> int:
    ...
    if out_path.exists():
        raise FileExistsError(f"refusing to overwrite v4 validation receipt: {out_path}")

    config_path = ROOT / "configs/moshi_h100_v4.yaml"
    ...
    run_dir = ROOT / "checkpoints/moshi_fa_100s_v4_text_embed"
    ...
    if not all(preconditions.values()):
        raise RuntimeError(f"v4 validation-pipeline preconditions failed: {preconditions}")
    ...
    for stage in validation_commands():
        ...
        if output.exists():
            raise FileExistsError(f"refusing to overwrite v4 stage output: {output}")
    ...
    report["next_stage"] = (
        ...
        else "v4 failed closed; design a new experiment without accessing the final test"
    )
    write_report(out_path, report)
