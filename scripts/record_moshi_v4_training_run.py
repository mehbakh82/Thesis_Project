#!/usr/bin/env python3
"""Create the fail-closed certificate for the bounded Moshi v4 run."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--run-dir",
        type=Path,
        default=Path("checkpoints/moshi_fa_100s_v4_text_embed"),
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/moshi_h100_v4.yaml"),
    )
    ...
    parser.add_argument(
        "--reevaluation",
        type=Path,
        default=Path("results/moshi_v4_validation_reevaluation.json"),
    )
    parser.add_argument(
        "--runtime-panel",
        type=Path,
        default=Path("results/moshi_v4_runtime_candidates.json"),
    )
    parser.add_argument(
        "--selection",
        type=Path,
        default=Path("results/moshi_v4_checkpoint_selection.json"),
    )
    parser.add_argument(
        "--validation-pipeline",
        type=Path,
        default=Path("results/hardware/moshi_v4_validation_pipeline.json"),
    )
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("results/hardware/moshi_h100_v4_training.json"),
    )
    ...
    if out_path.exists():
        raise FileExistsError(f"refusing to overwrite v4 training certificate: {out_path}")
    ...
    preflight_path = ROOT / "results/hardware/moshi_v4_preflight.json"
    probe_path = ROOT / "results/hardware/moshi_h100_v4_profile_probe.json"
    policy_path = ROOT / "configs/moshi_v4_embedding_policy.json"
    ...
    if first_timestamp is None or not elapsed_matches:
        raise ValueError("v4 journal lacks expected UTC timestamp/elapsed fields")
    ...
    if not isinstance(launch_config, dict):
        raise ValueError("launch-commit v4 config is not a mapping")
    ...
    embedding_parameter_sets = {
        tuple(row["embedding_parameters"]) for row in checkpoint_rows
    }
    adapter_sizes = {row["adapter_bytes"] for row in checkpoint_rows}
    selection_valid = (
        selection.get("schema_version") == 6
    ...
    requirements = {
        ...
        "preflight_probe_and_selective_policy_passed": (
            ...
            and (probe.get("embedding_policy") or {}).get("sha256")
            == sha256_file(policy_path)
        ),
        "v3_negative_finalized_without_test_access": (
            v3_certificate.get("status") == "passed"
    ...
    report: dict[str, Any] = {
        ...
        "experiment": {
            ...
            "changed_from_v3": {
                "lora_rank": {"v3": 128, "v4": 128},
                "upstream_ft_embed": {"v3": False, "v4": False},
                "selective_embedding_parameters": {
                    "v3": [],
                    "v4": ["depformer_text_emb.weight", "text_emb.weight"],
                },
                "candidate_horizon": {"v3": 500, "v4": 500},
            },
        },
        "service": {
        ...
        "next_stage": (
            ...
            else "v4 is a finalized negative result; do not access its final test"
        ),
    }
    ...
    if not passed:
        raise RuntimeError(f"v4 training certificate failed: {requirements}")
