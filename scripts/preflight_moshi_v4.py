#!/usr/bin/env python3
"""Fail-closed preflight for the bounded Moshi v4 experiment."""
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/moshi_h100_v4.yaml"))
    parser.add_argument(
        "--probe-config",
        type=Path,
        default=Path("configs/moshi_h100_v4_probe.yaml"),
    )
    parser.add_argument(
        "--embedding-policy",
        type=Path,
        default=Path("configs/moshi_v4_embedding_policy.json"),
    )
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("results/hardware/moshi_v4_preflight.json"),
    )
    ...
    if out_path.exists():
        raise FileExistsError(f"refusing to overwrite v4 preflight evidence: {out_path}")

    v3_config_path = ROOT / "configs/moshi_h100_v3.yaml"
    protocol_path = ROOT / "docs/MOSHI_V4_SELECTION_PROTOCOL.md"
    official_example_path = ROOT / "third_party/checkouts/moshi-finetune/example/moshi_7B.yaml"
    v3_selection_path = ROOT / "results/moshi_v3_checkpoint_selection.json"
    v3_runtime_path = ROOT / "results/moshi_v3_runtime_candidates.json"
    v3_certificate_path = ROOT / "results/hardware/moshi_h100_v3_training.json"
    ...
    v3_config = yaml_object(v3_config_path)
    official_example = yaml_object(official_example_path)
    policy = json_object(policy_path)
    v3_selection = json_object(v3_selection_path)
    v3_runtime = json_object(v3_runtime_path)
    v3_certificate = json_object(v3_certificate_path)
    split = json_object(split_path)

    estimate = selective_adapter_bytes(
        adapter_schema(v3_adapter_path),
    ...
    safety_bytes = 2 * GIB
    required_free_bytes = (
        estimate["estimated_adapter_bytes"]
        * (retained_adapter_count + transient_adapter_count)
        + safety_bytes
    )
    ...
    v3_pass_counts = [
        ...
        for candidate in v3_runtime.get("candidates") or []
    ]
    final_output_paths = (
        ROOT / "results/moshi_v4_checkpoint_selection.json",
    ...
    requirements = {
        ...
        "v3_failed_closed_without_test_access": (
            v3_selection.get("status") == "failed"
            and v3_selection.get("eligible_candidate_count") == 0
            and v3_selection.get("selected") is None
            and v3_selection.get("heldout_test_used_for_selection") is False
            and v3_certificate.get("status") == "passed"
            and v3_certificate.get("test_access_started") is False
            and v3_certificate.get("selection_outcome")
            == "scientific_negative_no_eligible_checkpoint"
            and (v3_certificate.get("artifacts") or {}).get("selection_sha256")
            == sha256_file(v3_selection_path)
            and v3_pass_counts == [0, 0, 0, 1, 0]
        ),
        "same_train_validation_and_optimization_invariants_as_v3": (
            experiment_invariants(config) == experiment_invariants(v3_config)
        ),
        "rank128_lora_and_broad_upstream_embedding_switch_unchanged": (
            lora
            == {"enable": True, "rank": 128, "scaling": 2.0, "ft_embed": False}
            and lora == (v3_config.get("lora") or {})
        ...
        "v4_run_directories_absent": not (
            (ROOT / str(config.get("run_dir"))).exists()
            or (ROOT / str(probe_config.get("run_dir"))).exists()
        ),
        "v4_final_outputs_absent": not any(path.exists() for path in final_output_paths),
    ...
    report: dict[str, Any] = {
        ...
        "experiment": {
            "name": "moshi_v4_text_embeddings_only",
        ...
        "artifacts": {
            ...
            "v3_selection_sha256": sha256_file(v3_selection_path),
            "v3_runtime_sha256": sha256_file(v3_runtime_path),
            "v3_certificate_sha256": sha256_file(v3_certificate_path),
            "split_report_sha256": sha256_file(split_path),
            "upstream_example_sha256": sha256_file(official_example_path),
            "rank128_source_adapter_sha256": sha256_file(v3_adapter_path),
        ...
        "next_stage": (
            "run the exact-shape one-step v4 probe with selective scope; do not access final-test data"
            if status == "passed"
            else "repair failed v4 preflight gates before any optimizer step"
        ),
    }
    write_report(out_path, report)
    if status != "passed":
        raise RuntimeError(f"Moshi v4 preflight failed: {requirements}")
    print(report)
    return 0
