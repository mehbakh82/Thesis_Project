#!/usr/bin/env python3
"""Run Moshi v4 final stages only after an eligible selection is committed."""
    ...
    return [
        {
            ...
            "test_access": False,
            "output": ROOT / "results/moshi_v4_adapter_validation.json",
            "command": [
                moshi_python,
                ...
                "--out",
                "results/moshi_v4_adapter_validation.json",
                "--runtime-device",
                "cuda",
                "--embedding-policy",
        ...
        },
        {
            ...
            "test_access": True,
            "output": ROOT / "results/moshi_v4_final_test.json",
            "command": [
                moshi_python,
                ...
                "--out",
                "results/moshi_v4_final_test.json",
                "--experiment-label",
                "v4",
            ],
        },
        {
            ...
            "test_access": True,
            "output": ROOT / "results/moshi_v4_final_runtime.json",
            "command": [
                moshi_python,
                ...
                "results/moshi_v2_split.json",
                "--out",
                "results/moshi_v4_final_runtime.json",
                "--experiment-label",
                "v4",
            ],
        },
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("results/hardware/moshi_v4_final_pipeline.json"),
    )
    args = parser.parse_args()
    out_path = (ROOT / args.out).resolve()
    if out_path.exists():
        raise FileExistsError(f"refusing to overwrite v4 final receipt: {out_path}")

    selection_path = ROOT / "results/moshi_v4_checkpoint_selection.json"
    validation_pipeline_path = ROOT / "results/hardware/moshi_v4_validation_pipeline.json"
    training_certificate_path = ROOT / "results/hardware/moshi_h100_v4_training.json"
    embedding_policy_path = ROOT / "configs/moshi_v4_embedding_policy.json"
    ...
    if not isinstance(selected, dict):
        raise RuntimeError("v4 final pipeline requires a selected checkpoint object")
    ...
    if not all(preconditions.values()):
        raise RuntimeError(f"v4 final-pipeline preconditions failed: {preconditions}")
    ...
    for stage in final_commands():
        ...
        if output.exists():
            raise FileExistsError(f"refusing to overwrite v4 final output: {output}")
