#!/usr/bin/env python3
"""Create the fail-closed certificate for the bounded Moshi v3 run."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--run-dir",
        type=Path,
        default=Path("checkpoints/moshi_fa_100s_v3_no_embed"),
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/moshi_h100_v3.yaml"),
    )
    parser.add_argument(
        "--launch-commit",
        default="d89d60fa176514ded5e52121a3d9e06fb4f57bc6",
    )
    parser.add_argument(
        "--service-unit",
        default="thesis-moshi-h100-v3-d89d60f.service",
    )
    parser.add_argument(
        "--invocation-id",
    ...
    parser.add_argument(
        "--reevaluation",
        type=Path,
        default=Path("results/moshi_v3_validation_reevaluation.json"),
    )
    parser.add_argument(
        "--runtime-panel",
        type=Path,
        default=Path("results/moshi_v3_runtime_candidates.json"),
    )
    parser.add_argument(
        "--selection",
        type=Path,
        default=Path("results/moshi_v3_checkpoint_selection.json"),
    )
    parser.add_argument(
        "--validation-pipeline",
        type=Path,
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("results/hardware/moshi_h100_v3_training.json"),
    )
    ...
    if out_path.exists():
        raise FileExistsError(f"refusing to overwrite v3 training certificate: {out_path}")
    ...
    if first_timestamp is None or not elapsed_matches:
        raise ValueError("v3 journal lacks expected UTC timestamp/elapsed fields")
    ...
    if not isinstance(launch_config, dict):
        raise ValueError("launch-commit v3 config is not a mapping")
    ...
    requirements = {
        ...
        "launch_commit_contains_exact_launcher": (
    ...
    report: dict[str, Any] = {
        ...
        "experiment": {
            ...
            "changed_from_v2": {
                "lora_rank": {"v2": 64, "v3": 128},
                "ft_embed": {"v2": True, "v3": False},
                "candidate_horizon": {"v2": 2000, "v3": 500},
            },
        },
        "service": {
        ...
        "next_stage": (
            ...
            else "v3 is a finalized negative result; do not access its final test"
        ),
    }
    ...
    if not passed:
        raise RuntimeError(f"v3 training certificate failed: {requirements}")
