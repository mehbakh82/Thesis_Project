#!/usr/bin/env python3
"""Evaluate every Moshi v3 checkpoint on the predeclared complete-prompt panel."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--profile-probe",
        type=Path,
        default=Path("results/hardware/moshi_h100_v3_profile_probe.json"),
    )
    parser.add_argument(
        "--protocol-document",
        type=Path,
        default=Path("docs/MOSHI_V3_SELECTION_PROTOCOL.md"),
    )
    parser.add_argument(
        "--out",
        type=Path,
    ...
    if out_path.exists():
        raise FileExistsError(f"refusing to overwrite v3 runtime evidence: {out_path}")
    ...
    preconditions = {
        "protocol_frozen_before_v3_training": (
    ...
    if not all(preconditions.values()):
        raise RuntimeError(f"v3 runtime-panel preconditions failed: {preconditions}")
    ...
    for candidate_index, step in enumerate(expected_steps, start=1):
        ...
        print(
            f"v3 runtime panel candidate {candidate_index}/{len(expected_steps)}: step={step}",
            flush=True,
        )
    ...
    if report["runtime_panel_evaluation_passes"] is not True:
        raise RuntimeError("v3 runtime-panel evaluation was incomplete")
