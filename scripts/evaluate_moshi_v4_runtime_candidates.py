#!/usr/bin/env python3
"""Evaluate every Moshi v4 checkpoint on the predeclared complete-prompt panel."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--profile-probe",
        type=Path,
        default=Path("results/hardware/moshi_h100_v4_profile_probe.json"),
    )
    parser.add_argument(
        "--protocol-document",
        type=Path,
        default=Path("docs/MOSHI_V4_SELECTION_PROTOCOL.md"),
    )
    parser.add_argument(
        "--embedding-policy",
        type=Path,
    ...
    if out_path.exists():
        raise FileExistsError(f"refusing to overwrite v4 runtime evidence: {out_path}")
    ...
    if not isinstance(declared_embedding_names, list) or not all(
        ...
        raise ValueError("v4 embedding policy has no valid trainable embedding list")
    ...
    preconditions = {
        "protocol_frozen_before_v4_training": (
        ...
        "v4_selective_embedding_policy_exact": (
            ...
            and (preflight_artifacts.get("embedding_policy_sha256")
                 == sha256_file(embedding_policy_path))
            and ((profile_probe.get("embedding_policy") or {}).get("sha256")
                 == sha256_file(embedding_policy_path))
        ),
        "reevaluation_passed": (
    ...
    if not all(preconditions.values()):
        raise RuntimeError(f"v4 runtime-panel preconditions failed: {preconditions}")
    ...
    for candidate_index, step in enumerate(expected_steps, start=1):
        ...
        print(
            f"v4 runtime panel candidate {candidate_index}/{len(expected_steps)}: step={step}",
            flush=True,
        )
    ...
    if report["runtime_panel_evaluation_passes"] is not True:
        raise RuntimeError("v4 runtime-panel evaluation was incomplete")
