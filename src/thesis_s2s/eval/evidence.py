...
def render_evidence_summary(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "| Trial | Context | LoRA rank | Embeddings trained | Candidates | "
            "Runtime passes per 9 | Minimum fixed-val loss | Deployment eligible |",
            "|---|---:|---:|---|---:|---|---:|---:|",
        ]
    )
    for trial in direct.get("trials") or []:
        ...
        lines.append(
            ...
            f"{config.get('lora_rank') or 'n/a'} | {embeddings} | "
            f"{trial.get('candidate_count', 0)} | "
            f"{', '.join(str(value) for value in pass_counts) if pass_counts else 'n/a'} | "
            f"{minimum_loss} | {'yes' if trial.get('deployment_eligible') else 'no'} |"
        )
    lines.extend(
        [
            "",
            ...
            "|---|---|---:|",
            f"| Detector | {detector.get('evidence_class')}; synthetic n="
            f"{detector.get('synthetic_n', 0)}, accuracy={detector.get('synthetic_accuracy')} | "
