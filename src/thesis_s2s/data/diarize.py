...
def diarize_episode_manifest(
    ...
    *,
    ...
    stats = {
        ...
        "complete": 0,
        "failed": 0,
        "automatic_multi_speaker_windows": 0,
        "automatic_multi_speaker_hours": 0.0,
        "aligned_hours": 0.0,
        "resumed_windows": len(done),
    }
    with (
        Path(in_jsonl).open("r", encoding="utf-8") as src,
        Path(out_jsonl).open("w", encoding="utf-8") as dst,
    ):
