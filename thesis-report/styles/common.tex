...
% Added for this thesis: professional table rules, multi-page tables, extended
% column types, and vector block diagrams. All must load before bidi/xepersian.
%
% bidi's array patch (array-xetex-bidi.def) is written against the 2024-06-01
% release of array and calls \UseMathForPositioningText, which the current
% release no longer provides. Requesting that dated rollback keeps \newcolumntype
% and paragraph columns working inside RTL tabulars.
...
% bidi/xepersian reset hyperref colours after they load. Internal links and
% citations were already blue; URLs defaulted to magenta unless restated here.
\hypersetup{
  colorlinks=true,
  linkcolor=blue,
  citecolor=blue,
...
% hyperref + bidi can split a \ref link's start/end markers across a line
% break, or reorder them inside an RTL paragraph. The PDF then paints one
% full-width annotation over two lines (seen on شکل~\ref{fig:arch},
% بخش~\ref{sec:early-attempt}, and جدول~\ref{tab:req-status} in the full
% report). An \mbox keeps those markers on one line. The copy is taken
% after hyperref has finished patching \ref, and \RenewDocumentCommand
% is required because \DeclareRobustCommand will not overwrite \ref.
\AddToHook{begindocument/end}{%
  \NewCommandCopy{\ThesisOrigRef}{\ref}%
  \RenewDocumentCommand{\ref}{s m}{%
    \IfBooleanTF{#1}%
      {\mbox{\ThesisOrigRef*{#2}}}%
      {\mbox{\ThesisOrigRef{#2}}}%
  }%
}
...
\SepMark{-}
% \small must sit outside the math shift, otherwise every itemize item logs
% "Command \small invalid in math mode" and the bullet keeps its normal size.
...
\newenvironment{alg}[2]
	{\begin{latin}\settextfont[Scale=0.95]{TeX Gyre Termes}
...
% \noindent is required: without it the paragraph indent (1.5em = 18pt at 12pt)
% is added to a \columnwidth-wide minipage and every alphabet heading reports an
% overfull box.
