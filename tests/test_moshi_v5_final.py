...
from scripts.run_moshi_v5_final import final_commands


def test_v5_final_pipeline_validates_selective_policy_before_test_access() -> None:
    ...
    assert stages[1]["command"][-2:] == ["--experiment-label", "v5"]
    assert stages[2]["command"][-2:] == ["--experiment-label", "v5"]
