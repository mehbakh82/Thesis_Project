...
| Direct speech LLM | Official Moshi LoRA trainer, pinned runtime, Persian stereo-response exporter, secure pinned local client, isolated environment, and H100 profiles are implemented and tested; all exact-revision base blobs pass byte/SHA-256 verification. The **6,754-pair / 108.584-hour** waiver-bound export and independent content audit pass, including 24/24 exact assistant re-syntheses. A real one-step H100 wiring smoke passed (loss 4.614331, 15.258 GB peak). Full adapter training now waits only for a safe-headroom exact-shape memory probe. Strict listening QA is explicitly waived, not represented as completed. The old 7 MB reconstruction artifact stays runtime-ineligible |
